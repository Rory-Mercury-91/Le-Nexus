/**
 * Gestion des statuts utilisateur AniList
 * Met à jour les statuts de lecture/visionnage pour manga et anime
 */

const { convertAniListReadingStatus, convertAniListUserStatus } = require('./anilist-transformers');
...
function updateMangaUserStatus(db, currentUser, serieId, mangaData) {
  ...
  // Convertir le statut utilisateur AniList vers le format de lecture
  // Si volumes_lus et chapitres_lus sont à 0, forcer "À lire" au lieu de "En cours"
  let statutLecture = convertAniListReadingStatus(mangaData.statut_perso);
  if (volumesLus === 0 && chapitresLus === 0 && statutLecture === 'En cours') {
    statutLecture = 'À lire';
  }

  // Log pour débogage
  console.log(`📚 Mise à jour statut manga ${serieId}: statut AniList="${mangaData.statut_perso}" → "${statutLecture}", ${volumesLus} volumes, ${chapitresLus} chapitres, score: ${mangaData.score_perso || 'N/A'}`);
...
function updateAnimeUserStatus(db, currentUser, animeId, animeData) {
  ...
  // Convertir le statut AniList vers le format de l'application
  // Si episodes_vus est à 0, forcer "À regarder" au lieu de "En cours"
  let statutVisionnage = convertAniListUserStatus(animeData.statut_perso);
  ...
  // Préparer episode_progress si AniList fournit des épisodes vus
  ...
  if (animeInfo) {
    ...
    if (isEnCours && hasSeenAllEpisodes && animeInfo.maj_disponible === 1) {
      ...
      console.log(`✅ Réinitialisation maj_disponible pour anime ${animeId} (synchronisation AniList: tous les épisodes vus)`);
    }
  }
}
