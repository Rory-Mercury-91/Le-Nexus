 ...
 */
function getUserImages(db, showId, userId) {
  const row = db.prepare('SELECT user_images FROM tv_show_user_data WHERE show_id = ? AND user_id = ?').get(showId, userId);
 ...
 */
function saveUserImages(db, showId, userId, images) {
  const imagesJson = JSON.stringify(images);
  db.prepare(`
    UPDATE tv_show_user_data 
    SET user_images = ?, updated_at = datetime('now')
    WHERE show_id = ? AND user_id = ?
  `).run(imagesJson, showId, userId);
}

/**
 * Enregistre les handlers IPC pour la gestion de la galerie d'images utilisateur des séries TV
 */
function registerTvShowGalleryHandlers(ipcMain, getDb, store, dialog, getMainWindow, getPathManager) {
  const getPaths = () => {
    ...
    // Créer un dossier galleries/tv-shows pour stocker les images utilisateur
    const galleriesDir = path.join(paths.base, 'galleries', 'tv-shows');
  ...
  ipcMain.handle('add-tv-show-user-image-url', async (event, showId, imageUrl, title = null) => {
    try {
      const db = getDb();
      if (!db) {
      ...
      const fileName = `tv_show_${showId}_user_${userId}_${Date.now()}`;
      ...
      const relativePath = `galleries/tv-shows/${finalFileName}`;

      // Ajouter l'image au JSON array
      const images = getUserImages(db, showId, userId);
      const newImage = {
      ...
      saveUserImages(db, showId, userId, images);

      console.log(`✅ Image utilisateur ajoutée depuis URL pour série ${showId}`);
      return {
        success: true,
        image: newImage,
      ...
      console.error('❌ Erreur add-tv-show-user-image-url:', error);
      return { success: false, error: error.message };
    }
  });

  // Ajouter une image depuis un fichier local
  ipcMain.handle('add-tv-show-user-image-file', async (event, showId, title = null) => {
    try {
      const db = getDb();
      if (!db) {
        return { success: false, error: 'Base de données non initialisée' };
      ...
      const mimeType = getMimeTypeFromExt(ext);
      const relativePath = `galleries/tv-shows/${fileName}`;
      ...
      console.log(`✅ Image utilisateur ajoutée depuis fichier pour série ${showId}`);
      return {
        success: true,
        image: newImage,
        filePath: relativePath,
      ...
      console.error('❌ Erreur add-tv-show-user-image-file:', error);
      return { success: false, error: error.message };
    }
  });

  // Récupérer les images utilisateur
  ipcMain.handle('get-tv-show-user-images', (event, showId) => {
    try {
      const db = getDb();
      if (!db) {
      ...
      console.error('❌ Erreur get-tv-show-user-images:', error);
      return { success: false, error: error.message, images: [] };
    }
  });

  // Supprimer une image utilisateur
  ipcMain.handle('delete-tv-show-user-image', (event, showId, imageId) => {
    try {
      const db = getDb();
      if (!db) {
        return { success: false, error: 'Base de données non initialisée' };
      ...
      console.log(`✅ Image utilisateur supprimée pour série ${showId}`);
      return { success: true };
    } catch (error) {
      console.error('❌ Erreur delete-tv-show-user-image:', error);
      return { success: false, error: error.message };
    }
  });
}

module.exports = { registerTvShowGalleryHandlers };
