...
(function() {
    ...
    console.log('═══════════════════════════════════════════════════');
    console.log('🎬 ADKAMI INTEGRATION v2.3.0 - Titre unifié corrigé');
    ...
    const checkUrlChange = () => {
        ...
        if (newUrl !== lastUrl) {
            ...
            const importBtn = document.getElementById('adkami-import-button');
            const episodeBtn = document.getElementById('adkami-episode-button');
            if (importBtn) importBtn.remove();
            if (episodeBtn) episodeBtn.remove();
            episodeSaved = null;
            
            // Re-initialiser après un délai
            setTimeout(() => {
                const currentUrl = window.location.pathname;
                if ((currentUrl.match(/\/anime\/\d+$/) || currentUrl.includes('/info')) && !currentUrl.match(/\/anime\/\d+\/\d+\/\d+\//)) {
                    console.log('📄 Mode: Import d\'anime (après changement URL)');
                    setTimeout(createImportButton, 500);
                } else if (currentUrl.match(/\/anime\/\d+\/\d+\/\d+\//)) {
                    console.log('🎬 Mode: Marquage d\'épisode (après changement URL)');
                    setTimeout(createEpisodeButton, 500);
                }
            }, 500);
        }
    };
    ...
    console.log('✅ Script ADKami initialisé');
