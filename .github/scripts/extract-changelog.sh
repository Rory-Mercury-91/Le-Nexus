...
if [ ! -f CHANGELOG.md ]; then
  ...
  awk -v ver="$VERSION_CLEAN" 'BEGIN { started=0 } /^## [0-9]{4}-[0-9]{2}-[0-9]{2}/ { if (match($0, /v?([0-9]+\.[0-9]+\.[0-9]+)/, arr)) { if (arr[1] == ver) { started = 1; next } else if (started) { exit } } } started { print }' CHANGELOG.md > changelog_extract.txt
fi
