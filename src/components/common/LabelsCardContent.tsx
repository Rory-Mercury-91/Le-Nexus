...
interface LabelsCardContentProps {
  itemId: number;
  onLabelsChange?: () => void;
  getLabels: (id: number) => Promise<Array<{ label: string; color: string }>>;
  getAllLabels: () => Promise<Array<{ label: string; color: string }>>;
  addLabel: (id: number, label: string, color: string) => Promise<{ success: boolean }>;
  removeLabel: (id: number, label: string) => Promise<{ success: boolean }>;
...
const LabelsCardContent: React.FC<LabelsCardContentProps> = ({ 
  itemId, 
  onLabelsChange,
  getLabels,
  getAllLabels,
  ...
  const loadLabels = async () => {
    try {
      const data = await getLabels(itemId);
      setLabels(data);
    } catch (error) {
  ...
  const handleAddLabel = async () => {
    ...
    setLoading(true);
    try {
      await addLabel(itemId, newLabelText.trim(), newLabelColor);
      await loadLabels();
      await loadAllLabels();
      setNewLabelText('');
  ...
  const handleRemoveLabel = async (label: string) => {
    ...
    setLoading(true);
    try {
      await removeLabel(itemId, label);
      await loadLabels();
  ...
  const handleAddExistingLabel = async (labelName: string) => {
    ...
    setLoading(true);
    try {
      await addLabel(itemId, labelToAdd.label, labelToAdd.color);
      await loadLabels();
...
export default LabelsCardContent;
