...
interface FieldCategory {
  title: string;
  icon: string;
  fields: { key: SeriesFieldKey; label: string }[];
}
...
const FIELD_CATEGORIES: FieldCategory[] = [
  ...
  },
  {
    ...
    icon: '🗂️',
    fields: [
      { key: 'metadata', label: 'Informations générales' },
  ...
  },
  {
    ...
    icon: '🔗',
    fields: [
      { key: 'streaming', label: 'Plateformes de diffusion' },
      { key: 'externalLinks', label: 'Liens externes (IMDb, réseaux sociaux...)' }
    ]
  }
];

const DEFAULTS: Record<SeriesFieldKey, boolean> = {
  banner: true,
  synopsis: true,
  nextEpisode: true,
  metadata: true,
...
interface SeriesDisplaySettingsModalProps {
  onClose: () => void;
  showToast: (options: { title: string; message?: string; type?: 'success' | 'error' | 'warning' | 'info'; duration?: number }) => void;
}

export default function SeriesDisplaySettingsModal({ onClose, showToast }: SeriesDisplaySettingsModalProps) {
  useModalEscape(onClose, false);

  const [prefs, setPrefs] = useState<Record<SeriesFieldKey, boolean>>(DEFAULTS);
  ...
  useEffect(() => {
    (async () => {
      try {
        const stored = await window.electronAPI.getSeriesDisplaySettings?.();
        if (stored) {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        showToast({
          ...
          message: 'Les préférences d’affichage des séries ont été enregistrées',
          type: 'success',
          duration: 2000
        });
      } catch (error) {
  ...
  const handleToggle = (key: SeriesFieldKey) => {
  ...
  const handleDeselectAll = () => {
    ...
    (Object.keys(cleared) as SeriesFieldKey[]).forEach((key) => {
      cleared[key] = false;
    });
    setPrefs(cleared);
  };
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '12px', color: 'var(--text-secondary)', margin: '4px 0 0' }}>
              Choisissez les sections visibles sur les fiches séries et saisons.
            </p>
          </div>
        </div>
        <button
