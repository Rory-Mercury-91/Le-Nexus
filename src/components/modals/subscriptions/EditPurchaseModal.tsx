...
export default function EditPurchaseModal({
  ...
  const [siteId, setSiteId] = useState<number | null>(purchase.site_id);
  const [siteName, setSiteName] = useState(purchase.site_name || '');
  const [purchaseDate, setPurchaseDate] = useState<string>(purchase.purchase_date);
  const [amount, setAmount] = useState<string>(purchase.amount.toString());
  const [devise, setDevise] = useState<string>((purchase as any).devise || 'EUR');
  const [creditsCount, setCreditsCount] = useState<string>(purchase.credits_count?.toString() || '');
  const [notes, setNotes] = useState(purchase.notes || '');
  const [proprietaireIds, setProprietaireIds] = useState<number[]>(
    purchase.proprietaires.map(p => p.id)
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const result = await window.electronAPI.oneTimePurchasesUpdate(purchase.id, {
        site_id: siteId,
      ...
      if (result?.success) {
        ...
        showToast({
          title: 'Erreur',
          message: result?.error || 'Impossible de modifier l\'achat',
          type: 'error'
        });
      }
    } catch (error) {
      ...
      showToast({
        title: 'Erreur',
        message: 'Impossible de modifier l\'achat',
        type: 'error'
      });
    } finally {
      setSaving(false);
    ...
    <>
      ...
      >
        <div style={{ padding: '24px' }}>
          <ModalHeader
            title="Modifier l'achat ponctuel"
            onClose={onClose}
          />
